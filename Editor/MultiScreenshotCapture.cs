...
namespace TalusKit.Editor
{
    internal static class ReflectionExtensions
    {
        ...
        internal static object FetchProperty(this Type type, string property) =>
                type.GetPropertyRecursive(property, true).GetValue(null, null);

        internal static object FetchProperty(this object obj, string property) =>
                obj.GetType().GetPropertyRecursive(property, false).GetValue(obj, null);

        internal static object CallMethod(this Type type, string method, params object[] parameters) => type
                .GetMethod(method, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)
                .Invoke(null, parameters);

        internal static object CallMethod(this object obj, string method, params object[] parameters) => obj.GetType()
        ...
        internal static object CreateInstance(this Type type, Type[] parameterTypes, object[] parameters) =>
                type.GetConstructor(parameterTypes).Invoke(parameters);
    ...
    public class MultiScreenshotCapture : EditorWindow
    {
        ...
        private List<Vector2> resolutions = new List<Vector2>
                { new Vector2(1024, 768) }; // Not readonly to support serialization
        private List<bool> resolutionsEnabled = new List<bool>
                { true }; // Same as above
        ...
        private void OnGUI()
        {
            ...
            if (targetCamera == TargetCamera.GameView)
            {
                ...
                if (captureOverlayUI && EditorApplication.isPlaying)
                {
                    ...
                    setTimeScaleToZero =
                            EditorGUILayout.ToggleLeft("Set timeScale to 0 during capture", setTimeScaleToZero);
            ...
            if (saveAsPNG && !captureOverlayUI && targetCamera == TargetCamera.GameView)
            {
                ...
                allowTransparentBackground =
                        EditorGUILayout.ToggleLeft("Allow transparent background", allowTransparentBackground);
            ...
            if (GUILayout.Button("Capture Screenshots"))
            {
                ...
                {
                    CaptureScreenshot((targetCamera == TargetCamera.GameView ? Camera.main
                            : SceneView.lastActiveSceneView.camera).pixelRect.size);
                }
        ...
        [MenuItem("TalusKit/Utility/Multi Screenshot Capture", priority = -998)]
        ...
        private void CaptureScreenshotWithoutUI(int width, int height)
        {
            ...
            try
            {
                ...
                screenshot = new Texture2D(renderTex.width, renderTex.height,
                    saveAsPNG && allowTransparentBackground ? TextureFormat.RGBA32 : TextureFormat.RGB24, false);
                ...
                File.WriteAllBytes(GetUniqueFilePath(renderTex.width, renderTex.height),
                    saveAsPNG ? screenshot.EncodeToPNG() : screenshot.EncodeToJPG(100));
            }
            finally
            {
        ...
        private static Type GetType(string type) => typeof(EditorWindow).Assembly.GetType("UnityEditor." + type);
